...
INFERENCE_LATENCY = Histogram(
    "vortexvision_inference_duration_seconds", "YOLO inference duration", ["stream_id"],
    buckets=[0.005, 0.01, 0.02, 0.03, 0.05, 0.075, 0.1, 0.2, 0.5],
)
FRAMES_PROCESSED = Counter("vortexvision_frames_processed_total", "Frames processed", ["stream_id"])
ANOMALY_EVENTS = Counter("vortexvision_anomaly_events_total", "Anomaly events", ["stream_id", "anomaly_type"])
...
def _load_models():
    ...
    try:
        ae_model.load_state_dict(
            torch.load(f"{settings.model_storage_path}/autoencoder_best.pt",
    ...
    anomaly_scorer = AnomalyScorer(ae_model, threshold=settings.ae_threshold,
                                   device=settings.yolo_device)

    tf_model = TemporalTransformer(seq_len=settings.seq_len)
    try:
        tf_model.load_state_dict(
            torch.load(f"{settings.model_storage_path}/transformer_best.pt",
                       map_location=settings.yolo_device)
        )
    ...
    behavioral_detector = BehavioralAnomalyDetector(
        tf_model, seq_len=settings.seq_len,
        threshold=settings.transformer_threshold, device=settings.yolo_device,
    )
...
def main():
    ...
    for stream_id, frame in consumer.frames():
        ...
        try:
            ...
            logger.error("Inference error on stream %s frame %d: %s", stream_id, frame_id, e)
            continue
        ...
        try:
            ...
            logger.error("Anomaly scoring error: %s", e)
        ...
        if ae_anomaly or tf_anomaly:
            ...
            # Classify the anomaly type from detection context
            ...
            ANOMALY_EVENTS.labels(stream_id=stream_id, anomaly_type=event.anomaly_type.value).inc()
            ...
            logger.info("Anomaly detected: %s on %s (ae=%.3f tf=%.3f)",
                        anomaly_type.value, stream_id, ae_score, tf_score)
