...
_CHECK_INTERVAL_SECONDS = int(os.getenv("DRIFT_CHECK_INTERVAL", "3600"))
_DRIFT_THRESHOLD = float(os.getenv("DRIFT_THRESHOLD", "0.3"))
_RETRAIN_WEBHOOK = os.getenv("RETRAIN_WEBHOOK_URL", "")


def _trigger_retrain_webhook(drift_score: float) -> None:
    ...
    try:
        ...
        logger.error("Retrain webhook error: %s", exc)


async def run_drift_monitor(state) -> None:
    from monitoring.evidently.drift_monitor import DriftMonitor  # noqa: PLC0415

    logger.info(
        "Drift monitor started (interval=%ds, threshold=%.2f)",
    ...
    if Path(_REFERENCE_PATH).exists():
        try:
            ...
            logger.warning("Could not load reference data: %s", exc)
    ...
    while True:
        ...
        try:
            ...
            logger.info(
                "Drift check: score=%.3f detected=%s",
                result["drift_score"],
